...
export class SDFPart extends Serializable {
   ...
   */
  static FIGURE_PART_32BIT_SIZE = 4 + 4 + 3 + 1 + 3*4;
  /**
   ...
   */
  constructor(sdf, start_point, end_point, smooth_min, extra_param) {
    super();
    this.sdf = sdf;
  ...
  toGLFloatArray() {
    ...
    buffer[0] = this.sdf;
    buffer[4] = this.start_point.x;
    buffer[5] = this.start_point.y;
    buffer[6] = this.start_point.z;
    buffer[8] = this.end_point.x;
    buffer[9] = this.end_point.y;
    buffer[10] = this.end_point.z;
    buffer[11] = this.smooth_min;
    buffer[12] = this.extra_param[0];
    buffer[16] = this.extra_param[1];
    buffer[20] = this.extra_param[2];
...
function convertSDFPartsToBuffer(parts) {
  ...
  const buffer = new Float32Array(
    parts.length * SDFPart.FIGURE_PART_32BIT_SIZE,
  );
