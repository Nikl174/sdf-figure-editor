...
export class FigureNode extends Serializable {
  ...
  /**
   * @param {string} name identifier for this node
   ...
   */
  static create = function (name, position_vector, param) {
    const fn = new FigureNode();
    /** @type {FigureNode[]}*/
    fn.childs = [];
  ...
  transformToList(start_point, vector_base) {
    ...
    if (this.childs != null) {
      ...
      for (const child of this.childs) {
        ...
        vec3.copy(new_start_vec, this.#end_point);
  ...
  /**
   * @brief Find a child by its defining name
   * @param {string} name name of the child to find
   * @return {FigureNode|null} the found node or null if not existing
   */
  findNodeByName(name) {
