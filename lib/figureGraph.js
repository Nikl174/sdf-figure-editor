...
export class FigureNode extends Serializable {
  ...
  transformToList(start_vec, vector_base) {
    ...
    const conv_trans_vec = sphericalToCatesianCoordinates(this.vector);
    mat3.transformVec3(
      conv_trans_vec,
      vector_base,
      conv_trans_vec,
    ...
    // calculate actual vector after end point calculation
    const vec = vec3.create();
    vec3.subtract(vec, this.#end_point, this.#start_point);

    // calculate new base by applying base base
    ...
    //TODO why/how to improve?:
    //swap rows to correct x and z axis wrong in sphericalToCatesianCoordinates()
    // swapMatrixRows(new_sphere_vec_base, 0, 2, 3);
    //deno-fmt-ignore
    const swap = mat3.fromValues(
      0, 0, 1,
      0, 1, 0,
      1, 0, 0,
    );
    mat3.multiply(new_sphere_vec_base, new_sphere_vec_base, swap);
