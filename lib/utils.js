 ...
 * @typedef {{radius: number, phi: number, theta: number}} SphereCoordinates
...
export class Serializable {
  ...
  static fromJSON(obj) {
    if (typeof obj !== "object") {
      throw new Error("Invalid JSON: expected an object! (Use JSON.parse(string) before)");
    }
 ...
 * @return {CartesianCoordinates} converted Cartesian one
 */
export function sphericalToCatesianCoordinates(sc) {
  const s_phi = Math.sin(sc.phi);
  const c_phi = Math.cos(sc.phi);
  const s_theta = Math.sin(sc.theta);
  const c_theta = Math.cos(sc.theta);
  return {
    x: sc.radius * s_theta * c_phi,
    y: sc.radius * s_theta * s_phi,
    z: sc.radius * c_theta,
  };
}
